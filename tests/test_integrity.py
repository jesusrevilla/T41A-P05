...
class TestDatabaseIntegrity(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.cur.execute("""
            ...
            VALUES (2, 1, CURRENT_DATE, 'Efectivo', 100, 100.00)
    ...
    def test_precio_unitario_integridad(self):
        ...
        self.cur.execute("""
            UPDATE articulo SET precio_unitario = 75.00 WHERE codigo = 100;
        """)

        # Verificar que el precio en detalle_de_factura no cambió
        self.cur.execute("""
            SELECT precio_unitario FROM detalle_factura
            WHERE sucursal = 2 AND numero_factura = 1 AND codigo_articulo = 100;
        """)
    ...
    def tearDownClass(cls):
        ...
        cls.cur.execute("DELETE FROM detalle_factura WHERE sucursal = 2 AND numero_factura = 1 AND codigo_articulo = 100;")
        cls.cur.execute("DELETE FROM factura WHERE sucursal = 2 AND numero_factura = 1;")
        cls.cur.execute("DELETE FROM cliente WHERE codigo = 100;")
        cls.cur.execute("DELETE FROM articulo WHERE codigo = 100;")
        cls.cur.close()
        cls.conn.close()
