...
class TestDatabaseIntegrity(unittest.TestCase):
    ...
    def test_precio_unitario_integridad(self):
        ...
        self.cur.execute("""
            UPDATE articulo SET precio_unitario = 75.00 WHERE codigo_del_articulo = 100;
        """)

        # Verificar que el precio en detalle_de_factura no cambió
        self.cur.execute("""
            SELECT precio_unitario_del_articulo FROM detalle_de_factura
            WHERE sucursal = 1 AND numero_de_factura = 1 AND codigo_de_articulo = 100;
        """)
    ...
    def tearDownClass(cls):
        ...
        cls.cur.execute("DELETE FROM detalle_de_factura WHERE codigo_de_articulo = 100;")
        cls.cur.execute("DELETE FROM factura WHERE numero_de_factura = 1;")
